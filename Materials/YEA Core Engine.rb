...
module YEA
  module CORE
    ...
    FONT_OUTLINE_COLOUR = Color.new(40, 40, 40, 160) # Default:   0,   0,   0, 128
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    TRANSPARENCY = 140   # Adjusts transparency of disabled items. Default: 160
    HP_CRISIS = 0.166     # When HP is considered critical. Default: 0.25
    MP_CRISIS = 0.166     # When MP is considered critical. Default: 0.25
...
class Window_Base < Window
  ...
  def translucent_alpha
    return 160 if SceneManager.scene_is?(Scene_Skill)
...
class Window_ItemList < Window_Selectable
  ...
  #--------------------------------------------------------------------------
  # overwrite method: draw_item_number #rvkd edit
  #--------------------------------------------------------------------------
...
#~     ap_cost = $game_party.in_battle ? BattleManager.actor.action_ap_cost(item) : item.ap_cost
#~     draw_text(rect, ap_cost, 2)
  end
