...
#==============================================================================
# Grid functions and calculations
#==============================================================================
module Grid
  #--------------------------------------------------------------------------
  ...
  def self.position(row, col)
    ...
    x_offset = Config::TileXOffset - (0.5 * w).to_i
    y_offset = Config::TileYOffset - (0.5 * h).to_i
  ...
  def self.coordinates_from_pos(x_pos, y_pos)
    max_row = Config::RadiusY
    max_col = Config::RadiusX
    w = Config::TileWidth
    base_x = Graphics.width / 2
    x_offset = Config::TileXOffset - (0.5 * w).to_i
  ...
  def self.troop_battler_coordinates(battler)
    ...
    xf = Config::TileWidth
    yf = Config::TileHeight
    mem_size = battler.grid_size
    cds = []

    case mem_size
    when 1
      cds << Grid.coordinates_from_pos(x_pos, y_pos)
    when 2
      cds << Grid.coordinates_from_pos(x_pos - xf/2, y_pos)
      cds << Grid.coordinates_from_pos(x_pos + xf/2, y_pos)
    when 4
      cds << Grid.coordinates_from_pos(x_pos - xf/2, y_pos - yf/2)
      cds << Grid.coordinates_from_pos(x_pos + xf/2, y_pos - yf/2)
      cds << Grid.coordinates_from_pos(x_pos - xf/2, y_pos + yf/2)
      cds << Grid.coordinates_from_pos(x_pos + xf/2, y_pos + yf/2)
    else
      cds.push(Grid.coordinates_from_pos(x_pos, y_pos))
    end
  ...
  def self.reset_troop_screen_xy(battler, coordinates)
    ...
    battler.screen_x = sum_x / coordinates.size + Config::UnitXOffset
    battler.screen_y = sum_y / coordinates.size + Config::UnitYOffset
